...
// ==============================================================
# 1 "C:/Users/ajaiswal/Downloads/project13-master/rng.c"
# 1 "C:/Users/ajaiswal/Downloads/project13-master/rng.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 147 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "C:/Users/ajaiswal/Downloads/project13-master/rng.c" 2
...
# 9 "C:/Users/ajaiswal/Downloads/project13-master/rng.c" 2
...
# 10 "C:/Users/ajaiswal/Downloads/project13-master/rng.c" 2
...
# 5 "C:/Users/ajaiswal/Downloads/project13-master/aes.h" 2
...
# 11 "C:/Users/ajaiswal/Downloads/project13-master/rng.c" 2
...
void print_aes256_struct() {
 int loop;
 print_aes256_struct_label0:for(loop=0;loop<32;loop++)
  printf("Key_enc[%d]=%d;\n",loop,DRBG_ctx.Key[loop]);
 print_aes256_struct_label5:for(loop=0;loop<16;loop++)
...
void fprint_aes256_struct(FILE *fp) {
 ...
 fprint_aes256_struct_label4:for(loop=0;loop<16;loop++)
...
void write_aes256_struct(unsigned char Key1[32], unsigned char V1[16], int reseed_counter1) {
 int loop;
 write_aes256_struct_label3:for(loop=0;loop<32;loop++)
...
# 54 "C:/Users/ajaiswal/Downloads/project13-master/rng.c"
int
seedexpander_init(AES_XOF_struct *ctx,
                  unsigned char *seed,
                  unsigned char *diversifier,
...
seedexpander(AES_XOF_struct *ctx, unsigned char *x, unsigned long xlen)
{
    ...
    offset = 0;
    seedexpander_label1:while ( xlen > 0 ) {
        ...
        seedexpander_label2:for (int i=15; i>=12; i--) {
            if ( ctx->ctr[i] == 0xff )
                ctx->ctr[i] = 0x00;
            else {
                ctx->ctr[i]++;
...
randombytes(unsigned char *x, unsigned long long xlen)
{
    ...
    sign_label1:while ( xlen > 0 ) {
        ...
        randombytes_label10:for (int j=15; j>=0; j--) {
...
# 251 "C:/Users/ajaiswal/Downloads/project13-master/rng.c"
        if ( xlen > 15 ) {

         int loop;
         randombytes_label11:for (loop=0;loop<16;loop++)
          x[i+loop] = block[loop];
            i += 16;
            xlen -= 16;
        }
         ...
         int loop;
         randombytes_label12:for (loop=0;loop<xlen;loop++)
               x[i+loop] = block[loop];
...
randombytes_sign(unsigned char *x, unsigned long long xlen)
{
    ...
    int i = 0;
        ...
        randombytes_sign_label13:for (int j=15; j>=0; j--) {
            if ( DRBG_ctx.V[j] == 0xff )
                DRBG_ctx.V[j] = 0x00;
            else {
                DRBG_ctx.V[j]++;
        ...
        AES256_ECB(DRBG_ctx.Key, DRBG_ctx.V, block);
# 301 "C:/Users/ajaiswal/Downloads/project13-master/rng.c"
         int loop;
         for (loop=0;loop<16;loop++)
          x[i+loop] = block[loop];
            i += 16;
            xlen -= 16;
# 315 "C:/Users/ajaiswal/Downloads/project13-master/rng.c"
    AES256_CTR_DRBG_Update(((void*)0), DRBG_ctx.Key, DRBG_ctx.V);
    DRBG_ctx.reseed_counter++;
